...
cell_type_lig_rec_frame <- function(interactions_compare,seurat_object,sample_replicates,sample_groups,metadata_grouping) {
  ...
  expr_dat <- Seurat::GetAssayData(seurat_object,slot="data",assay="RNA") %>%
    as.matrix() %>%
    t() %>%
    # deprecated code: as_tibble(.data,rownames="cell_barcodes")
    as_tibble(rownames="cell_barcodes", .name_repair = "minimal")
...
# deprecated code: sample1 <- suppressMessages(left_join(extracted_sample_groups,sample1,by=c(.data[[sample_groups]],.data[[sample_replicates]])))
sample1 <- suppressMessages(left_join(extracted_sample_groups,sample1,by=c(sample_groups,sample_replicates)))
...
# deprecated code: sample2 <- suppressMessages(left_join(extracted_sample_groups,sample2,by=c(.data[[sample_groups]],.data[[sample_replicates]])))
sample2 <- suppressMessages(left_join(extracted_sample_groups,sample2,by=c(sample_groups,sample_replicates)))
